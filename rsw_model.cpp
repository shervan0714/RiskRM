...
HighPerformanceRSW::HighPerformanceRSW(string task, int num_envs, double lambda_reg, 
...
// keeping this function just in case we need to print the loss later. 
// we don't actually need it for training now since we have analytical gradients.
double HighPerformanceRSW::compute_loss_internal(const Eigen::MatrixXd& X, const Eigen::VectorXd& y,
                                                  ...
                                                  const vector<Eigen::VectorXd>& all_env_weights, double current_penalty_weight) const {
    size_t N = X.rows();
    size_t D = X.cols();
    ...
    for (int e = 0; e < num_envs; ++e) {
        ...
        if (task == "classification") {
            ...
            for (size_t i = 0; i < N; ++i) {
                double p = 1.0 / (1.0 + exp(-preds(i)));
                p = clamp(p, 1e-15, 1.0 - 1e-15);
                ce_loss -= w(i) * (y(i) * log(p) + (1.0 - y(i)) * log(1.0 - p));
            }
...
void HighPerformanceRSW::fit(const Eigen::MatrixXd& X, const Eigen::VectorXd& y) {
    ...
    mt19937 global_gen(seed);
    normal_distribution<double> norm_dist(0.0, 1.0);

    for (int ep = 1; ep <= epochs; ++ep) {
        
        // --- STEP 1: setting up covariate-determining environments ---
        vector<Eigen::VectorXd> all_env_weights(num_envs, Eigen::VectorXd(N));
        for (int e = 0; e < num_envs; ++e) {
            // 1. creating a random projection vector
            Eigen::VectorXd projection = Eigen::VectorXd::NullaryExpr(D, [&](){ return norm_dist(global_gen); });
            
            for (size_t i = 0; i < N; ++i) {
                // 2. scoring the sample based on real market features (X)
                double score = X.row(i).dot(projection);
                
                // 3. passing it through sigmoid to get smooth weights between 0 and 1
                all_env_weights[e](i) = 1.0 / (1.0 + exp(-score)); 
            }
        }
        
        double penalty_weight = (ep > warmup_epochs) ? 1.0 : 0.0;

        // --- STEP 2: exact analytical gradients ---
        // totally removes those slow loops using matrix calculus. saves a lot of time.
        
        Eigen::VectorXd theta_eff = theta.cwiseProduct(mask);
        Eigen::VectorXd err = (X * theta_eff) - y;

        // 1. finding gradients for each specific environment
        ...
        // 3. invariance penalty gradient using the hessian-vector trick
        ...
        // 4. applying chain rule so we can separate gradients for theta and the mask (mu)
        ...
        // --- STEP 3: vectorized updates for momentum and velocity on theta ---
        m_t = beta1 * m_t + (1.0 - beta1) * grad_theta;
        v_t = beta2 * v_t + (1.0 - beta2) * grad_theta.cwiseAbs2();
        Eigen::VectorXd m_t_hat = m_t / (1.0 - pow(beta1, ep));
        Eigen::VectorXd v_t_hat = v_t / (1.0 - pow(beta2, ep));
        theta -= (lr_theta * m_t_hat.array() / (v_t_hat.array().sqrt() + eps_adam)).matrix();

        // --- STEP 4: updating momentum and tracking parameters for the mask space (mu) ---
        if (ep > warmup_epochs) {
            int mu_step = ep - warmup_epochs;
            m_m = beta1 * m_m + (1.0 - beta1) * grad_mu;
            v_m = beta2 * v_m + (1.0 - beta2) * grad_mu.cwiseAbs2();
            Eigen::VectorXd m_m_hat = m_m / (1.0 - pow(beta1, mu_step));
            Eigen::VectorXd v_m_hat = v_m / (1.0 - pow(beta2, mu_step));
            mu -= (lr_mu * m_m_hat.array() / (v_m_hat.array().sqrt() + eps_adam)).matrix();
        }

        // finally updating the active mask values
        mask = (1.0 + (-mu.array()).exp()).inverse().matrix();
    }
}

Eigen::VectorXd HighPerformanceRSW::predict(const Eigen::MatrixXd& X) const {
    // generating the final safe prediction using our calculated sparse mask
